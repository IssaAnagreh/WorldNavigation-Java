...
public class PlayerControllerMaster implements PlayerControllerInterface {
    ...
    public void init_commands() {
        ...
        c.put("orientation", playerController -> {
            this.myOrientation();
            if (this.hint) this.playerModel.notify_player("You can use <o> as a shortcut command" + this.HINT);
        });
        c.put("o", PlayerControllerInterface::myOrientation);
        c.put("location", playerController -> {
            this.myLocation();
            if (this.hint) this.playerModel.notify_player("You can use <loc> as a shortcut command" + this.HINT);
        });
        ...
        c.put("left", playerController -> {
            this.rotateLeft();
            if (this.hint) this.playerModel.notify_player("You can use <l> as a shortcut command" + this.HINT);
        });
        c.put("l", PlayerControllerInterface::rotateLeft);
        c.put("right", playerController -> {
            this.rotateRight();
            if (this.hint) this.playerModel.notify_player("You can use <r> as a shortcut command" + this.HINT);
        });
        c.put("r", PlayerControllerInterface::rotateRight);
        c.put("forward", playerController -> {
            move(PlayerControllerMaster.MoveParam.forward);
            if (this.hint) this.playerModel.notify_player("You can use <f> as a shortcut command" + this.HINT);
        });
        c.put("f", playerController -> move(PlayerControllerMaster.MoveParam.forward));
        c.put("backward", playerController -> {
            move(PlayerControllerMaster.MoveParam.backward);
            if (this.hint) this.playerModel.notify_player("You can use <b> as a shortcut command" + this.HINT);
        });
        c.put("b", playerController -> move(PlayerControllerMaster.MoveParam.backward));
        c.put("check", playerController -> {
            this.check();
            this.acquire_items();
            if (this.hint) this.playerModel.notify_player("You can use <c> as a shortcut command" + this.HINT);
        });
        c.put("c", playerController -> {
            this.check();
            this.acquire_items();
        });
        c.put("myItems", playerController -> {
            this.myItems();
            if (this.hint) this.playerModel.notify_player("You can use <items> as a shortcut command" + this.HINT);
        });
        c.put("items", PlayerControllerInterface::myItems);
        c.put("useKey", playerController -> {
            this.use_key();
            if (this.hint) this.playerModel.notify_player("You can use <key> as a shortcut command" + this.HINT);
        });
        ...
        c.put("switchLight", playerController -> {
            this.switchLights();
            if (this.hint) this.playerModel.notify_player("You can use <light> as a shortcut command" + this.HINT);
        });
        c.put("light", PlayerControllerInterface::switchLights);
        c.put("flashLight", playerController -> {
            this.flashLight();
            if (this.hint) this.playerModel.notify_player("You can use <flash> as a shortcut command" + this.HINT);
        });
